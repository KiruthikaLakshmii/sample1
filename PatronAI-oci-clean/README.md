...
```bash
# Teardown — removes OCI resources
bash teardown.sh
```
...
| `PUBLIC_HOST` | No | — | OCI VM public IP or DNS (no protocol) |
...
└── ghost-ai-scanner/                         # the actual product
    ...
    ├── oci-policy.json                       # OCI IAM policy for the runtime role
    │
    ├── src/                                  # business logic
    ...
    │   ├── migrate_data.sh                   # Migrate S3 + Grafana volume from AWS → OCI
